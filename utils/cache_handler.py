...
def fetch_carga_childs():
    ...
    return pd.read_parquet(f'http://72.60.252.104:7001/data/carga_nino.parquet', engine='pyarrow')

@st.cache_data(ttl=DEELAY_DATA,show_spinner="Cargando datos vd gestantes...")
def fetch_vd_gestantes():
    return pd.read_parquet(f'http://72.60.252.104:7001/data/actividad_gestante.parquet', engine='pyarrow')

@st.cache_data(ttl=DEELAY_DATA,show_spinner="Cargando datos gestantes...")
def fetch_carga_gestantes():
    return pd.read_parquet(f'http://72.60.252.104:7001/data/carga_gestantes.parquet', engine='pyarrow')

@st.cache_data(ttl=DEELAY_DATA,show_spinner="Cargando datos padron...")
def fetch_padron():
    return pd.read_parquet(f'http://72.60.252.104:7001/data/padron.parquet', engine='pyarrow')
